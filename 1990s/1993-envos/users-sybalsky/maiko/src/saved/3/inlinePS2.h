...
#define fn_section1(lbl,lpfx,nargs) \
  ...
  int RESTARGS;						\
	...
	xorb $3,%%al							\n\
	movb (%%eax),%%dl						\n\
	movl %%edx,%0		// fn_atom_index in edx from here.	\n\
	" : "=g" (fn_atom_index) : : "ax", "cx", "dx");			\
									\
    asm volatile("							\n\
	testl $16711680,%%edx	//fn_defcell = 				\n\
	je ." lpfx "118							\n\
//	addl %%edx,%%edx	// new atom case			\n\
	leal 8(,%%edx,2),%%eax	// 2* edx				\n\
	addl Lisp_world,%%ex						\n\
//	addl $8,%%edx							\n\
	jmp ." lpfx "119						\n\
	.align 4							\n\
." lpfx "118:								\n\
//	sall $2,%%edx		// old atom case			\n\
	leal 0(,%%edx,4),%%eax	// 2* edx				\n\
	addl Defspace,%%eax	// fn_defcell in eax from here.		\n\
." lpfx "119:								\n\
//	movl %%edx,%0	// to -92					\n\
    ...
    asm volatile("							\n\
	movl (%%eax),%%ecx						\n\
//	movl %%ecx,%0	// to -32 (must remain, it's used by C below)	\n\
	testl %%ecx,%%ecx	// if defcell_word>0			\n\
	jnl ." lpfx "120						\n\
...
." lpfx "127:								\n\
	...
	"g" (defcell_word) : "ax", "dx", "cx");				\
									\
    fast0_dispatcher();	/* not nextop0, to avoid C goto, so optimizer*/	\
			/* doesn't remove the following part as dead */	\
 									\
									\
									\
    /************************************************************/	\
    /*								*/	\
    /*  Fail case if the called fn isn't compiled code.  When	*/	\
    /*  we arrive here, fn_defcell is in %eax, and fn_atom_index*/	\
    /*  is in %edx -- we avoided storing them above to save	*/	\
    /*  time.  So store them, then punt out to the fail case.	*/	\
    /*								*/	\
    /************************************************************/	\
									\
   asm volatile("							\n\
	.align 4							\n\
." lpfx "120:								\n\
	movl %%eax,%0		// Save	fn_defcell			\n\
	movl %%edx,%1		// Save	fn_atom_index.			\n\
"	: "=g" (fn_defcell), "=g" (fn_atom_index));			\
									\
	{ /* it's not a CCODEP */					\
