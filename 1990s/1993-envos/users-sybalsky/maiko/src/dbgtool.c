/* @(#) dbgtool.c Version 1.4 (2/8/93). copyright venue & Fuji Xerox  */
static char *id = "@(#) dbgtool.c	1.4 2/8/93	(venue & Fuji Xerox)";
...
LispPTR get_fn_fvar_name(fnobj , offset)
  struct fnhead	*fnobj;
  DLword offset ;
  {
    NAMETABLE		*first_table;
...
LispPTR get_fvar_name(fx_addr68k , offset)
  ...
  {
#ifdef BIGVM
    get_fn_fvar_name(Addr68k_from_LADDR((fx_addr68k)->fnheader), offset);
#else
    get_fn_fvar_name(
	    Addr68k_from_LADDR(((int)(fx_addr68k)->hi2fnheader << 16) | (fx_addr68k)->lofnheader), offset);
#endif /* BIGVM */
  }/* end get_fvar_name */
