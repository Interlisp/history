/* @(#) inlnPS2.h Version 1.2 (5/27/92). copyright Venue & Fuji Xerox  */
...
#define Get_SBYTE_PCMAC1 Get_SBYTE_PCMAC1fn(pccache)
...
extern inline const unsigned int Get_SBYTE_PCMAC1fn (pccache)
unsigned int pccache;
 {
    register unsigned int word;
    asm("
...
#define fn_section1a(lbl,lpfx,nargs,oplen)				\
    asm volatile("							\n\
	testl $16711680,%%edx						\n\
	je ." lpfx "118							\n\
...
." lpfx "127:								\n\
	...
	movzwl 4(%%ecx),%%edx	// + LOCFNCELL-> startpc		\n\
/	movl %2,%%edx							\n\
	leal 1(%%ecx,%%edx),%%esi					\n\
/	movl %%edx,%%esi	//replace above 2 inst			\n\
/	movl %2,%%eax	//FuncObj = LOCFNCELL				\n\
	movl %%ecx,MachineState+20					\n\
	testl $67108864,8(%%ecx)	/ check for needs-byte-swapping		\n\
	jz  ." lpfx "swap	/ if so, go do it out of line.		\n\
...
inline const extern void opreturn()
  {
	...
	addl %%ecx,%%ecx					\n\
	addl Lisp_world,%%ecx					\n\
	movl %%ecx,MachineState+20	/ Set FuncObj		\n\
	movzwl -12(%%eax),%%eax					\n\
	leal 1(%%eax,%%ecx),%%esi				\n\
	testl	$67108864,8(%%ecx)	/ Is it swapped?	\n\
	jz	.opretswap		/ No. Go do it.		\n\
.oprc:	movzwl 2(%%ecx),%%edx					\n\
	addl %%edx,%%edx					\n\
